...
def verticalhorizontal_image(parameters=None, width=800, height=600, outline=20, background="white", **kwargs):
    """
    Examples
    ---------
...
def verticalhorizontal_parameters(difficulty=0, illusion_strength=0, size_min=0.5):
    ...
    right_coord, _, _ = _coord_line(x1=0.25, y1=0, length=right_length, angle=right_angle)
    right_x1, right_y1, right_x2, right_y2 = right_coord

    # Center y
    ys = np.array([left_y1, left_y2, right_y1, right_y2])
    ymax = np.max(ys)
    left_y1, left_y2, right_y1, right_y2 = ys - (ymax / 2)
    ...
    parameters = {"Illusion": "VerticalHorizontal",

...
def ebbinghaus_image(
    ...
    for side in ["Left", "Right"]:
        image = _ebbinghaus_image_draw(
            image, parameters, side=side, color_inner="red", color_outer="black"
        )
...
def _ebbinghaus_image_draw(
    image, p, side="Left", color_inner="red", color_outer="black"
):

    # Draw inner circle
    image = image_circle(
        image=image,
        size=p["Size_Inner_" + side],
        x=p["Position_" + side],
        y=0,
        color=color_inner,
    )
...
def ebbinghaus_parameters(
    difficulty=0, size_min=0.25, illusion_strength=0, distance=1, distance_auto=False
):
    # Size inner circles
    parameters = _delboeuf_parameters_sizeinner(
        difficulty=difficulty, size_min=size_min
    )
    ...
    l_outer_x, l_outer_y, l_distance_edges = _ebbinghaus_parameters_outercircles(
        x=position_left,
        y=0,
    ...
    r_outer_x, r_outer_y, r_distance_edges = _ebbinghaus_parameters_outercircles(
        x=position_right,
        y=0,
...
def _ebbinghaus_parameters_outercircles(
    x=0, y=0, size_inner=0.25, size_outer=0.3, n="auto"
):

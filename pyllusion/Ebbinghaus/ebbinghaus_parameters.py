...
from ..Delboeuf.delboeuf_parameters import (
    _delboeuf_parameters_sizeinner,
    _delboeuf_parameters_sizeouter,
)


def _ebbinghaus_parameters(
...
def _ebbinghaus_parameters_outercircles(
    x=0, y=0, size_inner=0.25, size_outer=0.3, n="auto", side="right"
):
    ...
    if side == "right":
        angle = np.deg2rad(np.linspace(180, 540, num=n, endpoint=False))
